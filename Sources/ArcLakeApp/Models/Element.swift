...
public struct ArcElement: Identifiable, Codable, Sendable {
    public var id: Int           // atomic number Z
    ...
    public var arcEdgeCircumference: Double {
        let diameter = atomicMass * 0.1
        return sqrt(pow(diameter * 3.0, 2.0))
    }
}

// MARK: — Standard periodic table grid position [atomicNumber: (period, group)]
// Empty cells use (period, 0) to indicate spacer
...
public final class ElementStore {
    ...
    // MARK: — Superactinide full names (elements.json truncates these)
    ...
    private func load() {
        guard let url = Bundle.main.url(forResource: "elements", withExtension: "json"),
              ...
              let raw = try? JSONDecoder().decode([ArcElementRaw].self, from: data)
        else {
        ...
        // Index 1 = Hydrogen (Z=1), index 2 = Helium (Z=2), etc.
        elements = raw.enumerated().compactMap { idx, r in
            guard idx > 0,                        // skip header
                  let n = r.neutrons as? Int,
                  let p = r.protons  as? Int,
                  let e = r.electrons as? Int
            else { return nil }

            let z = idx  // atomic number = array index
            // Use canonical names for superactinides
            let name   = ElementStore.superactinideNames[z]?.name   ?? r.elementName.trimmingCharacters(in: .whitespaces)
            ...
            return ArcElement(
                ...
                orbits: r.orbits, electronOrbits: r.electronOrbits,
                atomicMass: massFor(z: z),
                category: categoryFor(z: z),
                standardState: stateFor(z: z),
                meltingPoint: nil, boilingPoint: nil,
                density: nil, electronegativity: nil
            )
        }
        print("[ElementStore] Loaded \(elements.count) elements")
    }

    private struct ArcElementRaw: Codable {
        let elementName: String
        let neutrons: AnyCodable
        let protons: AnyCodable
        let electrons: AnyCodable
        let orbits: Int
        let electronOrbits: [Int]
    }
}

// Decode Int or String for the header row fields
private struct AnyCodable: Codable {
    ...
    static func as_(_ v: AnyCodable) -> Int? { v.value as? Int }
}

extension AnyCodable {
    static func ~=(lhs: AnyCodable, rhs: Int.Type) -> Bool { lhs.value is Int }
}
...
// Int? cast helper
private func asInt(_ a: AnyCodable) -> Int? { a.value as? Int }
extension ElementStore.ArcElementRaw {
    var neutrons: AnyCodable { neutrons }
    var protons: AnyCodable { protons }
    var electrons: AnyCodable { electrons }
}
...
private func massFor(z: Int) -> Double {
    let m: [Int:Double] = [1:1.008,2:4.003,3:6.941,4:9.012,5:10.811,6:12.011,
        7:14.007,8:15.999,9:18.998,10:20.180,11:22.990,12:24.305,13:26.982,
        14:28.086,15:30.974,16:32.065,17:35.453,18:39.948,19:39.098,20:40.078,
        26:55.845,29:63.546,47:107.868,79:196.967,92:238.029]
    ...
    static var builtinElements: [ArcElement] {
        (1...127).map { z in
            ArcElement(id:z, elementName:symbolFor(z:z), elementSymbol:symbolFor(z:z),
                      ...
                      orbits:max(1,(z-1)/18+1), electronOrbits:[z],
