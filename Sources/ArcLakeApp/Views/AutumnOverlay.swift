...
// MARK: — Ash Canvas Drawer — build178 logic (single tap handler, working sockets)
// MARK: — Ash Canvas Drawer v3
        ...
        VStack(spacing: 0) {
            ...
            GeometryReader { geo in
                ...
                ZStack {
                    ...
                    // Draw links
                    // ForEach+Path so SwiftUI re-renders each line live during node drag
                    ForEach(links) { lnk in
                        if let p1 = pos(lnk.fromId, sockPositions),
                           let p2 = pos(lnk.toId,   sockPositions) {
                            let sel = selectedLinkIds.contains(lnk.id)
                            Path { path in path.move(to: p1); path.addLine(to: p2) }
                                .stroke(sel ? Color.yellow : Color.cyan.opacity(0.45),
